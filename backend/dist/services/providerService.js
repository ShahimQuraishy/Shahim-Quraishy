...
const calcHash = (input) => createHash('sha256')
...
export class PgDataService {
    pool;
    retentionDays;
    constructor(pool, retentionDays) {
        this.pool = pool;
    ...
    async upsertProviders(records) {
        let inserted = 0;
        let updated = 0;
        let unchanged = 0;
        for (const record of records) {
            const hash = calcHash(record);
            const existing = await this.pool.query('SELECT id, data_hash FROM service_providers WHERE source_record_id = $1 AND datenquelle = $2 LIMIT 1', [record.sourceRecordId, record.datenquelle]);
            if (existing.rows[0] && existing.rows[0].data_hash === hash) {
                unchanged += 1;
                await this.pool.query('UPDATE service_providers SET letzte_aktualisierung = now(), last_seen_at = now(), aktiv = true WHERE id = $1', [existing.rows[0].id]);
                continue;
            }
            const values = [
    ...
    async runMaintenance() {
        const duplicateUpdate = await this.pool.query(`
      WITH ranked AS (
        SELECT id,
               row_number() OVER (
      ...
      RETURNING 1
    `);
        const staleUpdate = await this.pool.query(`
      ...
      RETURNING 1
    `);
        const anonymizeCalls = await this.pool.query(`
      UPDATE call_history
...
export class InMemoryDataService {
    ...
    async searchProviders(filters) {
        ...
        if (filters.city)
            data = data.filter((r) => r.ort.toLowerCase() === filters.city?.toLowerCase());
        if (filters.postalCode)
            data = data.filter((r) => r.plz === filters.postalCode);
        if (filters.category)
            data = data.filter((r) => r.kategorie.toLowerCase() === filters.category?.toLowerCase());
